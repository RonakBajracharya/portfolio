...
export default function PortfolioClient({
  ...
  // Hero entrance animation — fires once on mount
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    // Small tick so the CSS transition actually plays after first paint
  ...
  return (
    <>
      {/* ── Hero ──────────────────────────────────────────────────────────── */}
      <section id="home" className="relative px-6 pt-50 pb-24 halftone">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-16 items-center relative z-10">
          {/* Text column */}
          <div
            className={`space-y-6 transition-all duration-1000 order-2 md:order-1 ${
              isVisible
          ...
          >
            ...
            <div className="flex gap-4 pt-2">
              <Button
                onClick={() =>
                  config.hero.cvUrl.startsWith("data:")
                    ? (window.location.href = config.hero.cvUrl)
          ...
          {/* Portrait column */}
          <div
            className={`relative transition-all duration-1000 delay-200 order-1 md:order-2 ${
              isVisible
                ? "translate-x-0 opacity-100"
                : "translate-x-8 opacity-0"
            }`}
          >
            {config.hero.portraitUrl ? (
              <Image
                ...
                width={500}
                height={600}
                className="relative w-full max-w-sm mx-auto rounded-2xl grayscale"
                priority
              />
            ) : (
      ...
      <section
        id="education"
        className="px-6 py-24 border-y border-border bg-secondary/30"
      >
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-16">
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground tracking-widest uppercase mb-8">
                Education
              </h3>
            ...
            {config.skills.map((item) => (
              <div key={item.name}>
                ...
                <Progress
                  ...
                  className="h-1 bg-border [&>div]:bg-foreground [&>div]:transition-all [&>div]:duration-700"
                />
              </div>
            ))}
          </div>
      ...
      <section
        id="certifications"
        className="px-6 py-24 bg-secondary/30 border-y border-border"
      >
        <div className="max-w-6xl mx-auto">
          <h2 className="text-4xl font-bold text-center mb-16 tracking-tight">
            Certifications &amp; Achievements
          ...
          <div className="grid md:grid-cols-2 gap-16">
            {/* Certifications */}
            <div>
              <h3 className="text-sm font-semibold text-muted-foreground tracking-widest uppercase mb-8 flex items-center gap-2">
              ...
              <div className="space-y-4">
                {config.certifications.map((c) => (
                  <div
                    key={c.id}
                    className="border border-border rounded-xl p-5 hover:border-foreground/10 transition-all cursor-pointer bg-background"
...
// ---------------------------------------------------------------------------
// JCard — expandable Journey card (education / experience)
// ---------------------------------------------------------------------------
