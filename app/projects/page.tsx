        ...
        <div className="max-w-4xl mx-auto px-6 py-16">
          ...
          <div className="space-y-6">
            {loading ? [1,2,3].map(i => <div key={i} className="h-28 bg-[#262626] animate-pulse rounded-xl" />) : projects.length === 0 ? (
              ...
              <Link key={p.id} href={`/projects/${p.id}`} className="block border border-border rounded-xl p-6 hover:border-foreground/10 transition-all bg-background group">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {p.link && <a href={p.link} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-xs text-foreground underline underline-offset-4 hover:text-muted-foreground ml-auto">View &rarr;</a>}
                </div>
              </Link>
            ))}
          </div>
